...
export class Renderer {
  ...
  private readonly enemySprite: HTMLCanvasElement;
  private readonly enemyHitSprite: HTMLCanvasElement;
  private readonly ammoSprite: HTMLCanvasElement;
  private readonly healthSprite: HTMLCanvasElement;

  constructor(
    canvas: HTMLCanvasElement,
    private readonly level: LevelMap,
  ) {
  ...
  private drawWallColumn(screenX: number, start: number, end: number, hit: RayHit): void {
  ...
  private renderSprites(state: RenderState): void {
  ...
  private drawBillboard(job: SpriteJob, playerX: number, playerY: number, playerAngle: number): void {
  ...
  private renderWeapon(kick: number): void {
    const { context } = this;
  ...
  private renderMinimap(state: RenderState): void {
    const { context } = this;
    ...
    context.fillStyle = PALETTE.light;
    context.fillRect(
      originX + Math.floor(state.player.x * scale) - 1,
      originY + Math.floor(state.player.y * scale) - 1,
      3,
      3,
    );
  }

  private renderDamageFlash(intensity: number): void {
    const { context } = this;
    context.save();
  ...
  private renderModeOverlay(state: RenderState): void {
  ...
  private createEnemySprite(hit: boolean): HTMLCanvasElement {
  ...
  private createPickupSprite(kind: Pickup['kind']): HTMLCanvasElement {
    const canvas = document.createElement('canvas');
    canvas.width = 12;
    canvas.height = 12;
    const context = canvas.getContext('2d');
